...
RUN yum install -y \
    ...
    apr-devel apr-util-devel pcre-devel distcache-devel zlib-devel \
    libselinux-devel openssl-devel apr-util-ldap libxml2-devel libjpeg-devel \
    libpng-devel zlib-devel curl-devel libmcrypt libmcrypt-devel \
    readline readline-devel icu libicu libicu-devel \
    openssh-server httpd httpd-devel mysql mysql-server mysql-devel \
    wget make epel-release dpkg-devel dpkg-dev
